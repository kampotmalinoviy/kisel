...
// Перо для рисования графика
        graphicsStroke = new BasicStroke(2.0f, BasicStroke.CAP_BUTT,
                BasicStroke.JOIN_ROUND, 10.0f, new float[]{30, 10, 20, 10, 10, 10, 20, 10, 30}, 0.0f);
...
// Сохранить массив точек во внутреннем поле класса
        ...
        extr = new Double[graphicsData.length][2];
...
// Изменение любого параметра приводит к перерисовке области
    ...
    public void setShowExtr(boolean showExtr) {
        this.showExtr = showExtr;
        repaint();
    }
...
// Найти минимальное и максимальное значение функции
        ...
        for (int i = 0; i < graphicsData.length; i++) {
            ...
            if ((i != 0 && i != graphicsData.length - 1 && graphicsData[i][1] > graphicsData[i - 1][1] && graphicsData[i][1] > graphicsData[i + 1][1]) || (i != 0 && i != graphicsData.length - 1 && graphicsData[i][1] < graphicsData[i - 1][1] && graphicsData[i][1] < graphicsData[i + 1][1])) {
                extr[j][0] = graphicsData[i][0];
                extr[j][1] = graphicsData[i][1];
                j++;
            }
        }
...
// Отобразить график
    ...
    protected void paintExtr(Graphics2D canvas) {
// Выбрать линию для рисования графика
        canvas.setStroke(graphicsStroke);
...
// Россия/Беларусь дробная часть отделяется запятой
        ...
        for (Double[] point : extr) {
            if (point[0] != null && point[1] != null) {
                Point2D.Double extrPoint = xyToPoint(point[0], point[1]);
                canvas.fill(new Ellipse2D.Double(extrPoint.getX() - 3, extrPoint.getY() - 6, 10, 10));
                canvas.drawString("(" + df.format(point[0]) + ", " + df.format(point[1]) + ")", (float) extrPoint.getX() + 10, (float) extrPoint.getY() - 10);
            }
        }
    }
...
// Центр - в точке (x,y)
            ...
            String formattedRemainder = str.contains(".") ? str.split("\\.")[0] : "";
            for (char part : formattedRemainder.toCharArray()) {
                if (part == '2' || part == '4' || part == '6' || part == '8' || part == '0') {
...
// Задать эллипс по центру и диагонали
            ...
            GeneralPath rhomb = new GeneralPath();
            rhomb.moveTo(center.getX(), center.getY() - markerSize / 2);
            // Верхняя точка
            rhomb.lineTo(center.getX() + markerSize / 2, center.getY());
            // Правая точка
            rhomb.lineTo(center.getX(), center.getY() + markerSize / 2);
            // Нижняя точка
            rhomb.lineTo(center.getX() - markerSize / 2, center.getY());
            // Левая точка
            rhomb.closePath();
